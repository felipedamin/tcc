...
if [ $# -lt 8 ]; then
    ...
    echo "usage: sh homolog.sh -p <path to file to parse starting from src> -c <full qualified name of class to fuzz> -m <method name to fuzz> -t <time to fuzz(30s/1m/etc)>"
    exit 1;
fi

while getopts p:c:m:t: flag
do
    case "${flag}" in
        p) parseFile=${OPTARG};;
        c) fuzzClass=${OPTARG};;
